...
public class OutboundServiceRestemplateClient implements  InitiableServiceRestemplateClient{
    ...
    public String initTestData() {
        ResponseEntity<String> restExchange = restTemplate.exchange("http://zuulserver:5555/api/outbound/test-data/init",
                HttpMethod.POST, null, String.class);
        return restExchange.getBody();
    }

    public String initTestData(String name) {
        ResponseEntity<String> restExchange = restTemplate.exchange("http://zuulserver:5555/api/outbound/test-data/init/{name}",
