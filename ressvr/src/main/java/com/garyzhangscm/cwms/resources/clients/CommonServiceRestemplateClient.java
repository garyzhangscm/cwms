...
public class CommonServiceRestemplateClient implements  InitiableServiceRestemplateClient{
    ...
    public String initTestData() {
        ResponseEntity<String> restExchange = restTemplate.exchange("http://zuulserver:5555/api/common/test-data/init",
                HttpMethod.POST, null, String.class);
        return restExchange.getBody();
    }

    public String initTestData(String name) {
        ResponseEntity<String> restExchange = restTemplate.exchange("http://zuulserver:5555/api/common/test-data/init/{name}",
                HttpMethod.POST, null, String.class, name);
        return restExchange.getBody();
    }

    public String[] getTestDataNames() {
        ResponseBodyWrapper<String[]> responseBodyWrapper = restTemplate.exchange("http://zuulserver:5555/api/common/test-data",
