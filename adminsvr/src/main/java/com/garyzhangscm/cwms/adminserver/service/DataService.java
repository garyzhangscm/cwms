...
public class DataService {
     ...
     * @param production if this is a production company
     */
    public void initiateProductionData(String companyName,
                                       String warehouseName,
                                       String adminUsername,
                                       String requestUsername,
                                       Boolean production) throws JsonProcessingException {
        ...
        if (Objects.isNull(production)) {
            // default the company to production company
            production = true;
        }
        ...
        new Thread(() ->{
            ...
            try {
                Company company  = initiateCompany(companyName, savedDataInitialRequest.getProduction());
                ...
                logger.debug("System controller number are initiated");

                List<UnitOfMeasure> unitOfMeasures = initiateUnitOfMeasure(company.getId(), warehouse.getId());
     ...
     * @return
     */
    private List<UnitOfMeasure> initiateUnitOfMeasure(Long companyId, Long warehouseId) throws JsonProcessingException {
        List<UnitOfMeasure> unitOfMeasures = new ArrayList<>();

        UnitOfMeasure each = new UnitOfMeasure(companyId, warehouseId, "EA", "Each");
        each = commonServiceRestemplateClient.createUnitOfMeasure(each);
        logger.debug("each UOM created! id: {}",
                each.getId());

        UnitOfMeasure caseUom = new UnitOfMeasure(companyId, warehouseId, "CS", "Case");
        caseUom = commonServiceRestemplateClient.createUnitOfMeasure(caseUom);
        logger.debug("case UOM created! id: {}",
                caseUom.getId());

        UnitOfMeasure pallet = new UnitOfMeasure(companyId, warehouseId, "PL", "Pallet");
    ...
    private Company initiateCompany(String companyName, Boolean production) throws JsonProcessingException {
        ...
        company.setProduction(Objects.isNull(production) ? true : production);
