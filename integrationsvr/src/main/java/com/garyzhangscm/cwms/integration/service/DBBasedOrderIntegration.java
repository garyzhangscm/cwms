...
public class DBBasedOrderIntegration {
    ...
    private void setupMissingField(Order order, DBBasedOrder dbBasedOrder){
        ...
        // 1. warehouse
        if(Objects.isNull(order.getWarehouseId())) {
                order.setWarehouseId(
                        warehouseLayoutServiceRestemplateClient.getWarehouseByName(
                                dbBasedOrder.getWarehouseName()
                        ).getId()
                );
        }

        // 2. customer: bill to customer and ship to customer
        // Bill to customer is optional
        if(Objects.isNull(order.getBillToCustomerId()) &&
            Objects.nonNull(dbBasedOrder.getBillToCustomerName())) {
                order.setBillToCustomerId(
                        commonServiceRestemplateClient.getCustomerByName(
                                dbBasedOrder.getBillToCustomerName()
                        ).getId()
                );
        }
        // Ship to customer is optional
        if(Objects.isNull(order.getShipToCustomerId()) &&
                Objects.nonNull(dbBasedOrder.getShipToCustomerName())) {
            order.setShipToCustomerId(
                    commonServiceRestemplateClient.getCustomerByName(
        ...
        if(Objects.isNull(order.getCarrierId()) &&
                Objects.nonNull(dbBasedOrder.getCarrierName())) {
            carrier = commonServiceRestemplateClient.getCarrierByName(
                    dbBasedOrder.getCarrierName());
            order.setCarrierId(carrier.getId());
        }

        // 4. carrier service level
        // Carrier service level is optional
        if(Objects.isNull(order.getCarrierServiceLevelId()) &&
                Objects.nonNull(dbBasedOrder.getCarrierServiceLevelName())) {
            if (Objects.isNull(carrier) ) {
                ...
                carrier.getCarrierServiceLevels().forEach(carrierServiceLevel -> {
                    if (carrierServiceLevel.getName().equals(dbBasedOrder.getCarrierServiceLevelName())) {
                        order.setCarrierServiceLevelId(carrierServiceLevel.getId());
                    }
                });
            }
        }
        ...
        if(Objects.isNull(order.getClientId()) &&
                Objects.nonNull(dbBasedOrder.getClientName())) {

            order.setClientId(
                    commonServiceRestemplateClient.getClientByName(
                            dbBasedOrder.getClientName()
                    ).getId()
            );
        }
