...
public interface AllocationConfigurationRepository extends JpaRepository<AllocationConfiguration, Long>, JpaSpecificationExecutor<AllocationConfiguration> {
     ...
     * item. We will only change the allocation configuration in the specific warehouse
     * @param oldItemId
     * @param newItemId
     */
    @Transactional
    @Modifying
    @Query(value = "update allocation_configuration set item_id = :newItemId where item_id = :oldItemId  and warehouse_id = :warehouseId",
            nativeQuery = true)
